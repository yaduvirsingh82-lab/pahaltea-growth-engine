...
- Two real defects were found by that live run and fixed: Node's `fetch` aborts a non-streaming Ollama request after five minutes (the provider now streams NDJSON), and a model returning a non-UUID claim citation aborted the persistence transaction (citations are now shape-checked, and the prompt no longer uses an `id=` prefix a model can echo).
